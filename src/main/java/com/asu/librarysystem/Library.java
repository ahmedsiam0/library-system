...
//Note that we want to add a function to check if the string (phone number) contains digits only :)
    ...
    public static void writeLibrary() {
        try {
            ...
            for (Book obj : books) {
                write1.write((obj.getIdCounter()+",,,"+ obj.getId()+",,,"+obj.getTitle()+",,,"+obj.getAuthor()+",,,"+obj.getPublicationYear()+",,,"+obj.isStatus()+",,,"+obj.getPrice()+",,,"+ obj.getQuantity()+",,,"+obj.getDescreption()+",,,"+obj.getCoverPath()+"\n").getBytes());
    ...
    public static void readLibrary() {
        ...
        for(Borrower borrower : borrowers) {
            ...
            while (scanner.hasNextLine()) {
                ...
                Transaction transaction = new Transaction(Integer.valueOf(parts[1]), Integer.valueOf(parts[2]), Integer.valueOf(parts[3]), Integer.valueOf(parts[4]),Integer.valueOf(parts[5]));
