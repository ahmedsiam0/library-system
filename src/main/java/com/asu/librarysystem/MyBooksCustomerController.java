...
public class MyBooksCustomerController {
    ...
    private Customer whoUserNow(){
        Customer customer=searchCustomerByUserName(userName.getText());
        return customer;
    }
...
//            objName.method;
    ...
    public void putsBooks(Customer customer){
        ...
        try {
            for(int i=0;i<orderBooksArrayList.size();i++){
                ...
                booksCard.setData(orderBooksArrayList.get(i));
                if(colm==6){
                    colm=0;
                    row++;
                }
    ...
    private ArrayList<Book> arrayReadyToSearch(String word){
        ...
        ArrayList<Book> orderBooksArrayList=customer.arrayOFOrderBooks();
        ArrayList<Book>foundBooks=searchInArrayListBookByTitle(word,orderBooksArrayList);
        ArrayList<Book>foundBooks2=searchInArrayListBookByAuthor(word,orderBooksArrayList);
